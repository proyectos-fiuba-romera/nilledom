...
public class CreationHandler implements EditorMode {
    ...
    public void draw(DrawingContext drawingContext) {
        DiagramElement elem = editor.getDiagram().getChildAt(tmpPos.getX(), tmpPos.getY());
        if (isNestingCondition(elem, drawingContext)) {
            Rectangle2D bounds = cachedBounds;
            drawingContext
                .drawRectangle(bounds.getX() - 5, bounds.getY() - 5, bounds.getWidth() + 10,
                    bounds.getHeight() + 10, ACCEPT_COLOR, null);
        }
     ...
     */
    private boolean isNestingCondition(DiagramElement nester, DrawingContext drawingContext) {
        return nester.canNestElements() && nester.getAbsoluteBounds()
            .contains(getElementBounds(drawingContext));
    }
    ...
    private Rectangle2D getElementBounds(DrawingContext drawingContext) {
        ...
        cachedBounds.setRect(tmpPos.getX(), tmpPos.getY(), cachedBounds.getWidth(),
    ...
    private void drawSilhouette(DrawingContext drawingContext) {
        ...
        drawingContext
            .drawRectangle(bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight(),
                null);
    }
