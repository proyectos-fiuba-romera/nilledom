...
import com.nilledom.draw.Compartment.Alignment;
...
public final class ClassElement extends AbstractCompositeNode
    ...
    public void setShowStereotypes(boolean flag) {
        if (showStereotypes && !flag) {
            setHeight(getSize().getHeight() - mainCompartment.getSize().getHeight());
        }
    ...
    private FontType getMainLabelFontType() {
        return (classData.isAbstract()) ? FontType.ABSTRACT_ELEMENT : FontType.ELEMENT_NAME;
    }
    ...
    @Override public Dimension2D getMinimumSize() {
        return new DoubleDimension(calculateMinimumWidth(), calculateMinimumHeight());
    }
    ...
    private double calculateTotalHeight() {
        ...
        if (showAttributes) {
            compartmentHeightSum += attributesCompartment.getMinimumSize().getHeight();
        }
        if (showOperations) {
            compartmentHeightSum += operationsCompartment.getMinimumSize().getHeight();
        }
    ...
    private double calculateTotalWidth() {
        double maxwidth = Math.max(mainCompartment.getSize().getWidth(), getSize().getWidth());
        if (showAttributes) {
    ...
    private double calculateMinimumWidth() {
        ...
        if (showAttributes) {
            minimumWidth =
                Math.max(minimumWidth, attributesCompartment.getMinimumSize().getWidth());
        }
        if (showOperations) {
            minimumWidth =
                Math.max(minimumWidth, operationsCompartment.getMinimumSize().getWidth());
        }
        return minimumWidth;
    }
