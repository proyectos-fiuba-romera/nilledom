...
public abstract class AbstractConnection implements Connection {
     ...
     */
    private AffineTransform calculateRotationTransform(Line2D aLine, Point2D anchorPoint) {
    ...
    private Line2D getLastSegmentToNode2() {
        ...
        lastSegment.setLine(points.get(points.size() - 2), points.get(points.size() - 1));
        return lastSegment;
    }
    ...
    private void drawSegment(DrawingContext drawingContext, Line2D segment) {
        if (isDashed()) {
            drawingContext
                .drawDashedLine(segment.getX1(), segment.getY1(), segment.getX2(), segment.getY2());
        } else {
            drawingContext
