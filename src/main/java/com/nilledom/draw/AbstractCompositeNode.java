...
public abstract class AbstractCompositeNode extends AbstractNode implements CompositeNode {
    ...
    public DiagramElement getChildAt(double x, double y) {
        ...
        ListIterator<DiagramElement> iter = getChildren().listIterator(getChildren().size());
     ...
     */
    private DiagramElement getDeepestElement(DiagramElement element, double x, double y) {
            ...
            for (DiagramElement child : getChildren()) {
                if (child.intersects(bounds)) {
                    ...
                    if (child instanceof CompositeNode) {
                        ((CompositeNode) child)
                            .getChildrenInSpecifiedBounds(bounds, resultCollection);
                    }
                }
            }
        }
     ...
     * {@inheritDoc}
     */
    @Override protected void notifyNodeMoved() {
