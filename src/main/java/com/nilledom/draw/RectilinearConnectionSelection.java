...
public class RectilinearConnectionSelection extends ConnectionSelection {
     ...
     */
    public RectilinearConnectionSelection(DiagramOperations operations, Connection conn) {
     ...
     */
    private void dragConnectedSegmentPointBefore(int index, double diffx, double diffy) {
        if (index > 0) {
     ...
     * @param diffy the y amount to drag
     */
    private void dragConnectedSegmentPointAfter(int index, double diffx, double diffy) {
     ...
     */
    private void moveDragPoint(int indexInConnection, double diffx, double diffy) {
        ...
        if (indexInConnection == 0) {
            ...
            editpoints.get(mapToEditPointIndex(indexInConnection))
                .setLocation(p.getX() + diffx, p.getY() + diffy);
        }
    }
     ...
     * @param diffy             the y amount to move
     */
    private void moveSegmentPoint(int segmentPointIndex, int moveIndex, double diffx,
        double diffy) {
        ...
        if (isSegmentHorizontal(segmentPointIndex)) {
            editpoints.get(editMoveIndex).setLocation(point.getX(), point.getY() + diffy);
        } else {
            editpoints.get(editMoveIndex).setLocation(point.getX() + diffx, point.getY());
        }
    }
    ...
    void truncateSegmentToNode(int segmentIndex) {
        ...
        if (editSegmentIndex == 0) {
            getConnection().getNode1()
                .calculateIntersection(segment, editpoints.get(editSegmentIndex));
        } else if (editSegmentIndex == editpoints.size() - 2) {
            getConnection().getNode2()
                .calculateIntersection(segment, editpoints.get(editSegmentIndex + 1));
        }
    }
