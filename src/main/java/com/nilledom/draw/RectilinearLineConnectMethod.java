...
public final class RectilinearLineConnectMethod implements LineConnectMethod {
    ...
    private static RectilinearLineConnectMethod instance = new RectilinearLineConnectMethod();
     ...
     */
    public static RectilinearLineConnectMethod getInstance() {
        return instance;
    }
     ...
     * {@inheritDoc}
     */
    public void generateAndSetPointsToConnection(Connection conn, Node sourceNode, Node targetNode,
        Point2D source, Point2D dest) {
        RectilinearLineBuilder linebuilder = RectilinearLineBuilder.getInstance();
        List<Point2D> points =
            linebuilder.calculateLineSegments(source, dest, Orientation.HORIZONTAL);
    ...
    public void drawLineSegments(DrawingContext drawingContext, Point2D source, Point2D dest) {
        ...
        if (points.size() > 0) {
            ...
            for (int i = 1; i < points.size(); i++) {
                ...
                drawingContext.drawLine(lastPoint.getX(), lastPoint.getY(), nextPoint.getX(),
                    nextPoint.getY());
                lastPoint = nextPoint;
            }
        }
