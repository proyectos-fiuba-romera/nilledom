...
public class MultiSelection implements Selection {
     ...
     */
    public MultiSelection(DiagramOperations operations, List<DiagramElement> theElements) {
        editor = operations;
    ...
    public void stopDragging(double xcoord, double ycoord) {
        ...
        for (DiagramElement element : elements) {
            if (element instanceof Node) {
                ...
                addTranslateConnectionOperations(moveOperations, (Connection) element, transx,
                    transy);
            }
        }
     ...
     */
    private void addMoveNodeOperation(List<Command> moveOperations, Node node, double transx,
        double transy) {
        Point2D targetPos =
            new Point2D.Double(node.getAbsoluteX1() + transx, node.getAbsoluteY1() + transy);
     ...
     * @param transy         the y translation
     */
    private void addTranslateConnectionOperations(List<Command> moveOperations, Connection conn,
    ...
    public void draw(DrawingContext drawingContext) {
        if (isDragging()) {
            drawingContext
                .drawRectangle(tmpPos.getX(), tmpPos.getY(), bounds.getWidth(), bounds.getHeight(),
                    null);
        } else {
    ...
    public boolean contains(double xcoord, double ycoord) {
        for (Selection selection : selections) {
            if (selection.contains(xcoord, ycoord))
                return true;
        }
        return false;
    }
