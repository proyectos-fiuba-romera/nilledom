...
import com.nilledom.draw.GeometryUtil.Orientation;
...
public final class RectilinearLineBuilder {
     ...
     * Static instance.
     */
    private static RectilinearLineBuilder instance = new RectilinearLineBuilder();
     ...
     * @return the calculated line segments
     */
    private List<Point2D> calculateLineSegments(double x1, double y1, double x2, double y2,
        Orientation firstSegmentOrientation) {
    ...
    public List<Point2D> calculateLineSegments(Node node1, Node node2) {
        ...
        switch (direction) {
            ...
            case SE_NW:
                return calculateLineSegments(node1.getAbsoluteX1(), node1.getAbsCenterY(),
                    node2.getAbsCenterX(), node2.getAbsoluteY2(), Orientation.HORIZONTAL);
            case SW_NE:
                return calculateLineSegments(node1.getAbsoluteX2(), node1.getAbsCenterY(),
                    node2.getAbsCenterX(), node2.getAbsoluteY2(), Orientation.HORIZONTAL);
            case NW_SE:
                return calculateLineSegments(node1.getAbsoluteX2(), node1.getAbsCenterY(),
                    node2.getAbsCenterX(), node2.getAbsoluteY1(), Orientation.HORIZONTAL);
            case NE_SW:
            default:
                return calculateLineSegments(node1.getAbsoluteX1(), node1.getAbsCenterY(),
                    node2.getAbsCenterX(), node2.getAbsoluteY1(), Orientation.HORIZONTAL);
     ...
     */
    private List<Point2D> createHorizontalLineSegment(Node node1, Node node2, boolean leftToRight) {
        List<Point2D> result = new LinkedList<Point2D>();
        double y = calculateOverlapMiddlePoint(node1.getAbsoluteY1(), node1.getAbsoluteY2(),
            node2.getAbsoluteY1(), node2.getAbsoluteY2());
        if (leftToRight) {
     ...
     * @return the line segment as a list of Point2D
     */
    private List<Point2D> createVerticalLineSegment(Node node1, Node node2, boolean topToBottom) {
        List<Point2D> result = new LinkedList<Point2D>();
        double x = calculateOverlapMiddlePoint(node1.getAbsoluteX1(), node1.getAbsoluteX2(),
            node2.getAbsoluteX1(), node2.getAbsoluteX2());
        if (topToBottom) {
     ...
     */
    private void createTwoSegments(List<Point2D> points, double x1, double y1, double x2, double y2,
        Orientation firstSegmentOrientation) {
    ...
    public NodeDirection getNodeDirection(Node node1, Node node2) {
        ...
        if (node2.getAbsCenterX() >= node1.getAbsCenterX() && rangesOverlap(node1y1, node1y2,
            node2y1, node2y2)) {
            return NodeDirection.WEST_EAST;
        }
        if (node1.getAbsCenterX() > node2.getAbsCenterX() && rangesOverlap(node1y1, node1y2,
            node2y1, node2y2)) {
            return NodeDirection.EAST_WEST;
        }
        if (node2.getAbsCenterY() >= node1.getAbsCenterY() && rangesOverlap(node1x1, node1x2,
            node2x1, node2x2)) {
            return NodeDirection.NORTH_SOUTH;
        }
        if (node1.getAbsCenterY() > node2.getAbsCenterY() && rangesOverlap(node1x1, node1x2,
            node2x1, node2x2)) {
            return NodeDirection.SOUTH_NORTH;
        }

        // the diagonal directions
        if (node2.getAbsCenterX() > node1.getAbsCenterX() && node2.getAbsCenterY() > node1
            .getAbsCenterY()) {
            return NodeDirection.NW_SE;
        }
        if (node1.getAbsCenterX() > node2.getAbsCenterX() && node1.getAbsCenterY() > node2
            .getAbsCenterY()) {
     ...
     */
    private boolean rangesOverlap(double r1start, double r1end, double r2start, double r2end) {
        return GeometryUtil.getInstance().rangesOverlap(r1start, r1end, r2start, r2end);
    }
     ...
     * @return true if r1 and r2 overlap, false otherwise
     */
    private double calculateOverlapMiddlePoint(double r1start, double r1end, double r2start,
        double r2end) {
