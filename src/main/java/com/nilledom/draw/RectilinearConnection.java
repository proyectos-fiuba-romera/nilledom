...
public class RectilinearConnection extends AbstractConnection {
    ...
    public Selection getSelection(DiagramOperations operations) {
        ConnectionSelection selection = new RectilinearConnectionSelection(operations, this);
        return selection;
    }
     ...
     * {@inheritDoc}
     */
    @Override public void setPoints(List<Point2D> thePoints) {
        super.setPoints(thePoints);
        node1ConnectPoint = getPointInNodeCoordinates(getNode1(), thePoints.get(0));
        node2ConnectPoint =
            getPointInNodeCoordinates(getNode2(), thePoints.get(thePoints.size() - 1));
    }
    ...
    private void reconnectPulledOffNodes() {
        if (getPoints().size() <= 3) {
            ...
            reattachConnectionPoint(getNode2(), node2ConnectPoint, getPoints().size() - 1,
                getPoints().size() - 2);
     ...
     */
    private void reattachConnectionPoint(Node node, Point2D nodeConnectPoint, int indexAtNode,
        int indexInner) {
        ...
        if (wasPulledOff(node, nodeConnectPoint, getPoints().get(indexAtNode))) {
            ...
            Point2D absConnectPoint = getPointInAbsoluteCoordinates(node, nodeConnectPoint);
            // sets the first segment in parallel
            double diffx = 0, diffy = 0;
            if (GeometryUtil.getInstance().getSegmentOrientation(pointAtNode, pointInner)
                == Orientation.HORIZONTAL) {
                // the segment can only be moved in y direction
                diffy = absConnectPoint.getY() - pointAtNode.getY();
            } else if (GeometryUtil.getInstance().getSegmentOrientation(pointAtNode, pointInner)
            ...
            pointInner.setLocation(pointInner.getX() + diffx, pointInner.getY() + diffy);
        }
    }
     ...
     */
    private boolean wasPulledOff(Node node, Point2D connectPoint, Point2D cmpPoint) {
        Point2D absConnectPoint = getPointInAbsoluteCoordinates(node, connectPoint);
        return !GeometryUtil.getInstance().equals(absConnectPoint.getX(), cmpPoint.getX())
            || !GeometryUtil.getInstance().equals(absConnectPoint.getY(), cmpPoint.getY());
    }

    /**
     * {@inheritDoc}
     */
    @Override public void copyData(Connection conn) {
        super.copyData(conn);
        setPoints(
            RectilinearLineBuilder.getInstance().calculateLineSegments(getNode1(), getNode2()));
    }
