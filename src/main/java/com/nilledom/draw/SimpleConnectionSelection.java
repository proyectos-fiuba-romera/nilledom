...
public class SimpleConnectionSelection extends ConnectionSelection {
     ...
     */
    public SimpleConnectionSelection(DiagramOperations operations, Connection conn) {
    ...
    protected void startDragControlPoint(double xcoord, double ycoord) {
        ...
        if (dragPointIndex > 0 && dragPointIndex < connectionPoints.size() - 1) {
    ...
    protected void startDragSegment(double xcoord, double ycoord) {
        ...
        if (editedSegment != null) {
            editpoints.add(getConnection().getPoints().indexOf(editedSegment.getP2()), dragPoint);
    ...
    private void copyConnectionPointsToEditPoints() {
        ...
        editpoints.get(editpoints.size() - 1)
            .setLocation(getConnection().getNode2().getAbsCenterX(),
    ...
    protected void finishDragControlPoint(double xcoord, double ycoord) {
        ...
        if (index > 0 && index < editpoints.size() - 1) {
            Line2D outersegment =
                new Line2D.Double(editpoints.get(index - 1), editpoints.get(index + 1));
    ...
    private void setOuterEditPointsToNodeIntersections() {
        ...
        Line2D firstSegment = new Line2D.Double(editpoints.get(0), editpoints.get(1));
        getConnection().getNode1().calculateIntersection(firstSegment, editpoints.get(0));
        Line2D lastSegment = new Line2D.Double(editpoints.get(editpoints.size() - 2),
            editpoints.get(editpoints.size() - 1));
        getConnection().getNode2()
            .calculateIntersection(lastSegment, editpoints.get(editpoints.size() - 1));
    }
